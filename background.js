...
function testRunUpdateCallListener(details, tabId, tab) {
    ...
    if (configOptions[technologyId] === technologies.dotnet) {
        if (action == tsTiaApiActions.pause) {
            ...
            logWarningMessage('Pausing a test case is currently not supported. Coverage recorded until the "pause" event will be processed, subsequent coverage will be lost.');
        }
    ...
    const setTeamscaleSessionCallback = function (cookieArray) {
        for (const cookie of cookieArray) {
            if (!(cookie.name.includes('teamscale-session'))) {
                ...
                } else if (!teamscaleServer.pathname.replace(/\/$/, "").endsWith(cookie.path.replace(/^\/|\/$/g, ""))) {
                    continue;
                }
            }
